...
class ModelBpjsRanap extends CI_Model
{
  ...
  public function getPenilaianSoapi($no_rawat)
  {
    $this->db->select('pemeriksaan_ranap.penilaian');
    $this->db->from('pemeriksaan_ranap');
    //$this->db->join('reg_periksa ON pemeriksaan_ranap.no_rawat = reg_periksa.no_rawat', 'inner');
    $this->db->join('pegawai', 'pemeriksaan_ranap.nip=pegawai.nik', 'inner');
    $this->db->join('dokter ', 'pegawai.nik=dokter.kd_dokter', 'inner');
    $this->db->where('pemeriksaan_ranap.no_rawat', $no_rawat);
    $this->db->not_like('pemeriksaan_ranap.pemeriksaan', '::');
    $this->db->order_by('pemeriksaan_ranap.tgl_perawatan', 'desc');
    $this->db->order_by('pemeriksaan_ranap.jam_rawat', 'desc');
    $this->db->limit(1);
